...
Runs the whole stack (Postgres, SeaweedFS object storage, and the `ava` API server) in Docker
Compose, then drives it with `avactl` from your host. For the day-to-day dev loop (running `ava`
...
- `AVA_JWT_SECRET` - required, the server won't start without it. Generate one with
  `openssl rand -base64 32`.
- `AVA_BOOTSTRAP_ADMIN_EMAIL` - set this to the email you'll sign in with. On first boot (only
  while no global admin exists yet) ava grants that email global-admin, which is what lets you
  create the first business.

Leave `AVA_PUBLIC_BASE_URL` / `AVA_RP_ID` at their `localhost` defaults unless you're exposing this
...
docker compose logs -f ava   # watch it come up; Ctrl-C once you see "starting ava"
```

This builds `ava` from the repo's `Dockerfile`, starts Postgres and SeaweedFS, runs migrations
once (the `migrate` service), then starts the API server on `localhost:9090` (gRPC, what `avactl`
talks to) and `localhost:9091` (plain HTTP, the passkey login pages).

## 3. Install avactl

`avactl` isn't shipped in the Docker image - it's the client you run locally against the server
...
```sh
avactl config set-context dev --server localhost:9090 --insecure --business 0
avactl config use-context dev
avactl login      # opens your browser to localhost:9091/auth/start
```

Click **Create a passkey** and register with the same email as `AVA_BOOTSTRAP_ADMIN_EMAIL` - since
that email was pre-seeded as an admin, no invite token is needed for this first account (any
account after this one does need an invite: `avactl business invite create`).

```sh
avactl whoami
avactl business create --name "My Company"     # note the returned id
avactl config set-context dev --server localhost:9090 --insecure --business <id>
avactl config use-context dev
```
...
for you with `avactl ledger-account create` calls, one per account - it already knows this
...
```
Using avactl against my local context, create the rest of a standard QuickBooks-style chart of
...
That's exactly how the accounts in this guide's examples were created - live `avactl` calls
...
API). Ask it to run `avactl report trial-balance` afterward to confirm everything landed
with zero balances, and `avactl ledger-account list --all` to review the result before you start
...
`docker-compose.yml` stores Postgres's data in the `ava_db_data` Docker volume. **Don't point that
...
`~/Library/Mobile Documents/com~apple~CloudDocs/ava-backups/`, pruning to the last 30 by default:
...
way and survives sleep/reboot better). Save as `~/Library/LaunchAgents/com.ava.backup.plist`,
...
<plist version="1.0">
<dict>
  <key>Label</key><string>com.ava.backup</string>
  <key>ProgramArguments</key>
  <array>
    <string>/bin/bash</string>
    <string>-lc</string>
    <string>cd /Users/you/code/ava && ./scripts/backup-to-icloud.sh</string>
  </array>
  <key>StartCalendarInterval</key>
  <dict><key>Hour</key><integer>2</integer><key>Minute</key><integer>0</integer></dict>
  <key>StandardOutPath</key><string>/tmp/ava-backup.log</string>
  <key>StandardErrorPath</key><string>/tmp/ava-backup.log</string>
</dict>
</plist>
```

```sh
launchctl load ~/Library/LaunchAgents/com.ava.backup.plist
```

The `docker compose` service needs to be up at 2am for this to work (Docker Desktop launching at
login is enough - the `db` container itself only needs to be running, not `ava`).
...
gunzip -c "$HOME/Library/Mobile Documents/com~apple~CloudDocs/ava-backups/ava-<stamp>.sql.gz" \
  | docker compose exec -T db psql -U ava -d ava
docker compose up -d --build
```

### Attachments

File attachments (`context attach`) live in SeaweedFS's `ava_seaweed_data` volume, not Postgres -
...
```sh
docker run --rm -v ava_seaweed_data:/data:ro \
  -v "$HOME/Library/Mobile Documents/com~apple~CloudDocs/ava-backups":/backup \
