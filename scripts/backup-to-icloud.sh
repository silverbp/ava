#!/usr/bin/env bash
# Dumps the ava Postgres database to a timestamped, gzipped file inside
...
dest="${1:-$HOME/Library/Mobile Documents/com~apple~CloudDocs/ava-backups}"
keep="${2:-30}"

mkdir -p "$dest"
stamp="$(date +%Y%m%d-%H%M%S)"
out="$dest/ava-$stamp.sql.gz"
tmp="$out.partial"
trap 'rm -f -- "$tmp"' EXIT # clean up a failed run's partial file - the mv below outruns this on success

docker compose exec -T db pg_dump -U ava -d ava | gzip > "$tmp"
mv "$tmp" "$out" # atomic rename - iCloud only ever sees the finished file

# Prune old backups, keeping the most recent $keep.
ls -1t "$dest"/ava-*.sql.gz 2>/dev/null | tail -n "+$((keep + 1))" | while IFS= read -r old; do
  rm -f -- "$old"
done
