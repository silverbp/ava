...
var paymentNoun = resource.Noun{
	...
	Columns: []resource.Column{
		resource.Int("ID", (*avav1.Payment).GetId),
		resource.Str("NUMBER", (*avav1.Payment).GetPaymentNumber),
		resource.Str("TYPE", (*avav1.Payment).GetPaymentType),
		resource.Money("AMOUNT", (*avav1.Payment).GetAmount),
		resource.Str("METHOD", (*avav1.Payment).GetPaymentMethod),
		resource.Bool("POSTED", func(p *avav1.Payment) bool { return p.LedgerTransactionId != nil }),
		resource.Str("INVOICES", func(p *avav1.Payment) string {
...
func newPaymentCmd() *cobra.Command {
	...
	root.AddCommand(
		newListCmd(paymentNoun, func(r run) ([]proto.Message, error) {
			resp, err := avav1.NewPaymentServiceClient(r.conn).ListPayments(r.ctx, &avav1.ListPaymentsRequest{BusinessId: r.businessID})
			return toMessages(resp.GetPayments()), err
		}),
		newGetCmd(paymentNoun, func(r run, id int64) (proto.Message, error) {
			resp, err := avav1.NewPaymentServiceClient(r.conn).GetPayment(r.ctx, &avav1.GetPaymentRequest{Id: id})
...
func newPaymentVoidCmd() *cobra.Command {
	...
	cmd := newMutateCmd(paymentNoun, "void", resource.Doc{
		...
		resp, err := avav1.NewPaymentServiceClient(r.conn).VoidPayment(r.ctx, &avav1.VoidPaymentRequest{Id: id, ReversalDate: reversalDate})
		return resp.GetPayment(), err
	})
...
func newPaymentCreateCmd() *cobra.Command {
	...
	cmd := newCreateCmd(paymentNoun, resource.Doc{
		...
		resp, err := avav1.NewPaymentServiceClient(r.conn).CreatePayment(r.ctx, &avav1.CreatePaymentRequest{
