...
var invoiceNoun = resource.Noun{
	...
	Columns: []resource.Column{
		resource.Int("ID", (*avav1.Invoice).GetId),
		resource.Str("NUMBER", (*avav1.Invoice).GetInvoiceNumber),
		resource.Str("TYPE", (*avav1.Invoice).GetInvoiceType),
		resource.Str("STATUS", (*avav1.Invoice).GetStatus),
		resource.Money("TOTAL", (*avav1.Invoice).GetTotalAmount),
		resource.Money("BALANCE_DUE", (*avav1.Invoice).GetBalanceDue),
		resource.Bool("POSTED", func(i *avav1.Invoice) bool { return i.LedgerTransactionId != nil }),
		resource.Int("VERSION", (*avav1.Invoice).GetResourceVersion),
	},
}
...
func newInvoiceCmd() *cobra.Command {
	...
	listCmd := newListCmd(invoiceNoun, func(r run) ([]proto.Message, error) {
		resp, err := avav1.NewInvoiceServiceClient(r.conn).ListInvoices(r.ctx, &avav1.ListInvoicesRequest{BusinessId: r.businessID, IncludeAll: includeAll})
	...
	root.AddCommand(
		...
		newGetCmd(invoiceNoun, func(r run, id int64) (proto.Message, error) {
			resp, err := avav1.NewInvoiceServiceClient(r.conn).GetInvoice(r.ctx, &avav1.GetInvoiceRequest{Id: id})
			return resp.GetInvoice(), err
		}, func(r run, id int64) ([]byte, error) {
			resp, err := avav1.NewInvoiceServiceClient(r.conn).GetInvoicePdf(r.ctx, &avav1.GetInvoicePdfRequest{Id: id})
...
func newInvoiceStatusCmd(verb string, doc resource.Doc, status string, reversalDate func(r run) (*typepb.Date, error)) *cobra.Command {
	return newVersionedMutateCmd(invoiceNoun, verb, doc, func(r run, id, resourceVersion int64) (proto.Message, error) {
		...
		resp, err := avav1.NewInvoiceServiceClient(r.conn).UpdateInvoiceStatus(r.ctx, &avav1.UpdateInvoiceStatusRequest{Id: id, Status: status, ResourceVersion: resourceVersion, ReversalDate: date})
		return resp.GetInvoice(), err
	})
}
...
func newInvoiceCancelCmd() *cobra.Command {
	...
	cmd := newInvoiceStatusCmd("cancel", resource.Doc{
		...
		Examples: []resource.Example{
			{Cmd: "avactl invoice cancel 42"},
			{Cmd: "avactl invoice cancel 42 --date 2026-02-01", Desc: "invoice is in a closed period - post the reversal in the open one"},
		},
...
func newInvoiceCreateCmd() *cobra.Command {
	...
	cmd := newCreateCmd(invoiceNoun, resource.Doc{
		...
		var lineItems []*avav1.NewDocumentLineItem
		if len(rawLines) > 0 {
			var err error
		...
		resp, err := avav1.NewInvoiceServiceClient(r.conn).CreateInvoice(r.ctx, &avav1.CreateInvoiceRequest{
...
func newInvoiceUpdateCmd() *cobra.Command {
	...
	cmd := newVersionedMutateCmd(invoiceNoun, "update", resource.Doc{
		...
		resp, err := avav1.NewInvoiceServiceClient(r.conn).UpdateInvoice(r.ctx, &avav1.UpdateInvoiceRequest{
			Id:              id,
...
func newInvoiceUpdateLinesCmd() *cobra.Command {
	...
	cmd := newVersionedMutateCmd(invoiceNoun, "update-lines", resource.Doc{
		...
		resp, err := avav1.NewInvoiceServiceClient(r.conn).UpdateInvoiceLineItems(r.ctx, &avav1.UpdateInvoiceLineItemsRequest{
			Id:              id,
			LineItems:       lineItems,
			ResourceVersion: resourceVersion,
		})
