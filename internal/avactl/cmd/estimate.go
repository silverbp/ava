...
var estimateNoun = resource.Noun{
	...
	Columns: []resource.Column{
		resource.Int("ID", (*avav1.Estimate).GetId),
		resource.Str("NUMBER", (*avav1.Estimate).GetEstimateNumber),
		resource.Str("STATUS", (*avav1.Estimate).GetStatus),
		resource.Money("TOTAL", (*avav1.Estimate).GetTotalAmount),
		resource.Int("VERSION", (*avav1.Estimate).GetResourceVersion),
	},
}
...
func newEstimateCmd() *cobra.Command {
	...
	listCmd := newListCmd(estimateNoun, func(r run) ([]proto.Message, error) {
		resp, err := avav1.NewEstimateServiceClient(r.conn).ListEstimates(r.ctx, &avav1.ListEstimatesRequest{BusinessId: r.businessID, IncludeAll: includeAll})
	...
	root.AddCommand(
		...
		newGetCmd(estimateNoun, func(r run, id int64) (proto.Message, error) {
			resp, err := avav1.NewEstimateServiceClient(r.conn).GetEstimate(r.ctx, &avav1.GetEstimateRequest{Id: id})
			return resp.GetEstimate(), err
		}, func(r run, id int64) ([]byte, error) {
			resp, err := avav1.NewEstimateServiceClient(r.conn).GetEstimatePdf(r.ctx, &avav1.GetEstimatePdfRequest{Id: id})
...
func newEstimateStatusCmd(verb, summary, status string) *cobra.Command {
	return newVersionedMutateCmd(estimateNoun, verb, resource.Doc{Summary: summary}, func(r run, id, resourceVersion int64) (proto.Message, error) {
		resp, err := avav1.NewEstimateServiceClient(r.conn).UpdateEstimateStatus(r.ctx, &avav1.UpdateEstimateStatusRequest{Id: id, Status: status, ResourceVersion: resourceVersion})
		return resp.GetEstimate(), err
	})
}
...
func newEstimateCreateCmd() *cobra.Command {
	...
	cmd := newCreateCmd(estimateNoun, resource.Doc{
		...
		resp, err := avav1.NewEstimateServiceClient(r.conn).CreateEstimate(r.ctx, &avav1.CreateEstimateRequest{
...
func newEstimateUpdateLinesCmd() *cobra.Command {
	...
	cmd := newVersionedMutateCmd(estimateNoun, "update-lines", resource.Doc{
		...
		resp, err := avav1.NewEstimateServiceClient(r.conn).UpdateEstimateLineItems(r.ctx, &avav1.UpdateEstimateLineItemsRequest{
			Id:              id,
			LineItems:       lineItems,
			ResourceVersion: resourceVersion,
		})
