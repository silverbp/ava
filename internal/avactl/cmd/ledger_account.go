...
var ledgerAccountNoun = resource.Noun{
	...
	Columns: []resource.Column{
		resource.Int("ID", (*avav1.LedgerAccount).GetId),
		resource.Str("CODE", (*avav1.LedgerAccount).GetCode),
		resource.Str("NAME", (*avav1.LedgerAccount).GetName),
		resource.Int("TYPE", (*avav1.LedgerAccount).GetAccountTypeId),
		resource.Bool("SYSTEM", (*avav1.LedgerAccount).GetIsSystem),
		resource.OptInt("PARENT", func(a *avav1.LedgerAccount) *int32 { return a.ParentAccountId }),
		resource.Bool("ACTIVE", (*avav1.LedgerAccount).GetIsActive),
		resource.Int("VERSION", (*avav1.LedgerAccount).GetResourceVersion),
	},
}
...
func newLedgerAccountCmd() *cobra.Command {
	...
	listCmd := newListCmd(ledgerAccountNoun, func(r run) ([]proto.Message, error) {
		resp, err := avav1.NewLedgerAccountServiceClient(r.conn).ListLedgerAccounts(r.ctx, &avav1.ListLedgerAccountsRequest{BusinessId: r.businessID})
		if err != nil {
			return nil, err
		}
	...
	root.AddCommand(
		...
		newGetCmd(ledgerAccountNoun, func(r run, id int64) (proto.Message, error) {
			resp, err := avav1.NewLedgerAccountServiceClient(r.conn).GetLedgerAccount(r.ctx, &avav1.GetLedgerAccountRequest{Id: int32(id)})
		...
		newVersionedMutateCmd(ledgerAccountNoun, "deactivate", resource.Doc{Summary: "Deactivate a ledger account"}, func(r run, id, resourceVersion int64) (proto.Message, error) {
			resp, err := avav1.NewLedgerAccountServiceClient(r.conn).DeactivateLedgerAccount(r.ctx, &avav1.DeactivateLedgerAccountRequest{Id: int32(id), ResourceVersion: resourceVersion})
			return resp.GetAccount(), err
		}),
	)
	return root
...
func newLedgerAccountCreateCmd() *cobra.Command {
	...
	cmd := newCreateCmd(ledgerAccountNoun, resource.Doc{
		...
		resp, err := avav1.NewLedgerAccountServiceClient(r.conn).CreateLedgerAccount(r.ctx, &avav1.CreateLedgerAccountRequest{
...
func newLedgerAccountUpdateCmd() *cobra.Command {
	...
	cmd := newVersionedMutateCmd(ledgerAccountNoun, "update", resource.Doc{
		...
		resp, err := avav1.NewLedgerAccountServiceClient(r.conn).UpdateLedgerAccount(r.ctx, &avav1.UpdateLedgerAccountRequest{
			Id:                        int32(id),
