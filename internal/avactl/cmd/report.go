...
func reportClient(r run) avav1.ReportingServiceClient { return avav1.NewReportingServiceClient(r.conn) }

func trialBalanceReport() reportSpec {
	return reportSpec{
		use: "trial-balance", summary: "Trial balance as of a date", example: "avactl report trial-balance --as-of 2026-01-31",
		asOf: true,
		pdf: func(r run, d reportDates) ([]byte, error) {
			resp, err := reportClient(r).GetTrialBalancePdf(r.ctx, &avav1.GetTrialBalancePdfRequest{BusinessId: r.businessID, AsOf: d.asOf})
			return resp.GetContent(), err
		},
		fetch: func(r run, d reportDates) (proto.Message, error) {
			resp, err := reportClient(r).GetTrialBalance(r.ctx, &avav1.GetTrialBalanceRequest{BusinessId: r.businessID, AsOf: d.asOf})
			return resp.GetTrialBalance(), err
		},
		print: typedPrinter(func(w io.Writer, tb *avav1.TrialBalance) {
...
func balanceSheetReport() reportSpec {
	return reportSpec{
		use: "balance-sheet", summary: "Balance sheet as of a date", example: "avactl report balance-sheet --as-of 2026-01-31",
		asOf: true,
		pdf: func(r run, d reportDates) ([]byte, error) {
			resp, err := reportClient(r).GetBalanceSheetPdf(r.ctx, &avav1.GetBalanceSheetPdfRequest{BusinessId: r.businessID, AsOf: d.asOf})
			return resp.GetContent(), err
		},
		fetch: func(r run, d reportDates) (proto.Message, error) {
			resp, err := reportClient(r).GetBalanceSheet(r.ctx, &avav1.GetBalanceSheetRequest{BusinessId: r.businessID, AsOf: d.asOf})
			return resp.GetBalanceSheet(), err
		},
		print: typedPrinter(func(w io.Writer, bs *avav1.BalanceSheet) {
...
func incomeStatementReport() reportSpec {
	return reportSpec{
		use: "income-statement", summary: "Income statement (P&L) over a date range", example: "avactl report income-statement --start 2026-01-01 --end 2026-01-31",
		period: true,
		pdf: func(r run, d reportDates) ([]byte, error) {
			resp, err := reportClient(r).GetIncomeStatementPdf(r.ctx, &avav1.GetIncomeStatementPdfRequest{BusinessId: r.businessID, PeriodStart: d.start, PeriodEnd: d.end})
			return resp.GetContent(), err
		},
		fetch: func(r run, d reportDates) (proto.Message, error) {
			resp, err := reportClient(r).GetIncomeStatement(r.ctx, &avav1.GetIncomeStatementRequest{BusinessId: r.businessID, PeriodStart: d.start, PeriodEnd: d.end})
			return resp.GetIncomeStatement(), err
		},
		print: typedPrinter(func(w io.Writer, is *avav1.IncomeStatement) {
			section := func(title string, lines []*avav1.IncomeStatementLine, totalLabel string, total *avav1.Decimal) {
...
func generalLedgerReport() reportSpec {
	var account int32
	return reportSpec{
		use: "general-ledger", summary: "General-ledger detail for one account over a date range", example: "avactl report general-ledger --account 10 --start 2026-01-01 --end 2026-01-31",
		...
		pdf: func(r run, d reportDates) ([]byte, error) {
			resp, err := reportClient(r).GetGeneralLedgerPdf(r.ctx, &avav1.GetGeneralLedgerPdfRequest{BusinessId: r.businessID, AccountId: account, PeriodStart: d.start, PeriodEnd: d.end})
			return resp.GetContent(), err
		},
		fetch: func(r run, d reportDates) (proto.Message, error) {
			resp, err := reportClient(r).GetGeneralLedger(r.ctx, &avav1.GetGeneralLedgerRequest{BusinessId: r.businessID, AccountId: account, PeriodStart: d.start, PeriodEnd: d.end})
			return resp.GetGeneralLedger(), err
		},
		print: typedPrinter(func(w io.Writer, gl *avav1.GeneralLedger) {
...
func customerStatementReport() reportSpec {
	var contact int64
	return reportSpec{
		use: "customer-statement", summary: "Invoice/payment activity, running balance, and aging for one contact", example: "avactl report customer-statement --contact 5 --start 2026-01-01 --end 2026-01-31",
		period: true, startDefault: "0001-01-01",
		...
		pdf: func(r run, d reportDates) ([]byte, error) {
			resp, err := reportClient(r).GetCustomerStatementPdf(r.ctx, &avav1.GetCustomerStatementPdfRequest{ContactId: contact, PeriodStart: d.start, PeriodEnd: d.end})
			return resp.GetContent(), err
		},
		fetch: func(r run, d reportDates) (proto.Message, error) {
			resp, err := reportClient(r).GetCustomerStatement(r.ctx, &avav1.GetCustomerStatementRequest{ContactId: contact, PeriodStart: d.start, PeriodEnd: d.end})
			return resp.GetStatement(), err
		},
		print: typedPrinter(func(w io.Writer, st *avav1.CustomerStatement) {
			fmt.Fprintf(w, "%s\n", st.GetContactName())
