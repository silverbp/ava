...
var contactNoun = resource.Noun{
	...
	Columns: []resource.Column{
		resource.Int("ID", (*avav1.Contact).GetId),
		resource.Str("NUMBER", (*avav1.Contact).GetContactNumber),
		resource.Str("NAME", (*avav1.Contact).GetName),
		resource.Bool("CUSTOMER", func(c *avav1.Contact) bool { return c.GetCustomer() != nil }),
		resource.Bool("VENDOR", func(c *avav1.Contact) bool { return c.GetVendor() != nil }),
		resource.Bool("ACTIVE", (*avav1.Contact).GetIsActive),
		resource.Int("VERSION", (*avav1.Contact).GetResourceVersion),
	},
}
...
func newContactCmd() *cobra.Command {
	...
	listCmd := newListCmd(contactNoun, func(r run) ([]proto.Message, error) {
		resp, err := avav1.NewContactServiceClient(r.conn).ListContacts(r.ctx, &avav1.ListContactsRequest{BusinessId: r.businessID, IncludeInactive: includeInactive})
	...
	root.AddCommand(
		...
		newGetCmd(contactNoun, func(r run, id int64) (proto.Message, error) {
			resp, err := avav1.NewContactServiceClient(r.conn).GetContact(r.ctx, &avav1.GetContactRequest{Id: id})
		...
		newVersionedMutateCmd(contactNoun, "deactivate", resource.Doc{Summary: "Deactivate a contact"}, func(r run, id, resourceVersion int64) (proto.Message, error) {
			resp, err := avav1.NewContactServiceClient(r.conn).DeactivateContact(r.ctx, &avav1.DeactivateContactRequest{Id: id, ResourceVersion: resourceVersion})
			return resp.GetContact(), err
		}),
	)
	return root
...
func newContactUpdateCmd() *cobra.Command {
	...
	cmd := newVersionedMutateCmd(contactNoun, "update", resource.Doc{
		...
		resp, err := avav1.NewContactServiceClient(r.conn).UpdateContact(r.ctx, &avav1.UpdateContactRequest{
			Id:                  id,
