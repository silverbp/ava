...
// than in production.
//
// Postgres comes from AVA_TEST_DSN when set (an existing database the tests
...
func TestMain(m *testing.M) {
	...
	dsn := os.Getenv("AVA_TEST_DSN")
	var stop func()
	if dsn == "" {
		var err error
		dsn, stop, err = startEmbeddedPostgres()
		if err != nil {
			fmt.Fprintf(os.Stderr, "starting embedded postgres: %v\n(set AVA_TEST_DSN to use an existing database, or run with -short to skip integration tests)\n", err)
			os.Exit(1)
		}
	}
...
func startEmbeddedPostgres() (dsn string, stop func(), err error) {
	...
	dataDir, err := os.MkdirTemp("", "ava-test-pg-")
	if err != nil {
		return "", nil, err
	}
	pg := embeddedpostgres.NewDatabase(embeddedpostgres.DefaultConfig().
		Port(port).
		Username("ava").Password("ava").Database("ava").
		DataPath(dataDir).
		Logger(io.Discard))
	...
	return fmt.Sprintf("postgres://ava:ava@127.0.0.1:%d/ava?sslmode=disable", port), stop, nil
}
