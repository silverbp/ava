...
func TestGetCustomerStatement_RequiresCustomerRole(t *testing.T) {
	...
	plain := must(contacts.CreateContact(ctx, &avav1.CreateContactRequest{
		BusinessId: tc.businessID, ContactNumber: "C-1", Name: "Not A Customer",
	})).GetContact()

	_, err := reports.GetCustomerStatement(ctx, &avav1.GetCustomerStatementRequest{
		ContactId: plain.GetId(), PeriodStart: dateOf(2026, 1, 1), PeriodEnd: dateOf(2026, 12, 31),
	})
	wantCode(t, err, codes.FailedPrecondition)

	_, err = reports.GetCustomerStatementPdf(ctx, &avav1.GetCustomerStatementPdfRequest{
		ContactId: plain.GetId(), PeriodStart: dateOf(2026, 1, 1), PeriodEnd: dateOf(2026, 12, 31),
	})
	wantCode(t, err, codes.FailedPrecondition)
}
...
func TestGetCustomerStatement_ReflectsManualLedgerEntry(t *testing.T) {
	...
	must(invoices.CreateInvoice(ctx, &avav1.CreateInvoiceRequest{
	...
	must(txns.CreateLedgerTransaction(ctx, &avav1.CreateLedgerTransactionRequest{
		BusinessId: tc.businessID, TransactionDate: dateOf(2026, 1, 15), Description: &desc,
		Entries: []*avav1.NewLedgerEntry{
			{AccountId: writeOff, DebitAmount: dec("25.00")},
			{AccountId: custAR, CreditAmount: dec("25.00")},
		},
	}))

	stmt := must(reports.GetCustomerStatement(ctx, &avav1.GetCustomerStatementRequest{
...
func TestGetCustomerStatement_UnpostedPaymentReflected(t *testing.T) {
	...
	pay := must(payments.CreatePayment(ctx, &avav1.CreatePaymentRequest{
		...
		Applications: []*avav1.PaymentApplicationInput{{InvoiceId: inv.GetId(), AppliedAmount: dec("100.00")}},
	})).GetPayment()
