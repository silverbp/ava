...
func eqAmount(t *testing.T, field string, got *avav1.Decimal, want string) {
	t.Helper()
	if got == nil {
...
func balanced(t *testing.T, txn *avav1.LedgerTransaction) decimal.Decimal {
	t.Helper()
...
func TestDocumentFlow(t *testing.T) {
	...
	_, err := items.CreateItem(ctx, &avav1.CreateItemRequest{BusinessId: tc.businessID, ItemCode: "BAD", Name: "Bad", RetailPrice: dec("1"), DefaultLedgerAccountId: &arContainer})
	...
	supplies := must(items.CreateItem(ctx, &avav1.CreateItemRequest{BusinessId: tc.businessID, ItemCode: "SUPPLIES", Name: "Supplies", RetailPrice: dec("50.00"), DefaultLedgerAccountId: &expense})).GetItem()

	// A line may not reference another business's item (or a nonexistent one).
	_, err = estimates.CreateEstimate(ctx, &avav1.CreateEstimateRequest{BusinessId: tc.businessID, CustomerId: customer.GetId(), EstimateDate: dateOf(2026, 1, 1), ExpirationDate: dateOf(2026, 2, 1),
		LineItems: []*avav1.NewDocumentLineItem{{ItemId: 1 << 40, LineNumber: 1}}})
	wantCode(t, err, codes.InvalidArgument)

	// --- estimate -> invoice ---------------------------------------------
	est := must(estimates.CreateEstimate(ctx, &avav1.CreateEstimateRequest{
		BusinessId: tc.businessID, CustomerId: customer.GetId(), EstimateDate: dateOf(2026, 1, 1), ExpirationDate: dateOf(2026, 2, 1),
		LineItems: []*avav1.NewDocumentLineItem{{ItemId: consult.GetId(), LineNumber: 1, Quantity: dec("2")}},
	})).GetEstimate()
	...
	listed := must(estimates.ListEstimates(ctx, &avav1.ListEstimatesRequest{BusinessId: tc.businessID, IncludeAll: true})).GetEstimates()
	...
	posting := must(txns.GetLedgerTransaction(ctx, &avav1.GetLedgerTransactionRequest{Id: inv.GetLedgerTransactionId()})).GetTransaction()
	...
	_, err = txns.ReverseLedgerTransaction(ctx, &avav1.ReverseLedgerTransactionRequest{Id: posting.GetId()})
	wantCode(t, err, codes.FailedPrecondition)

	// --- payment: apply, post, void --------------------------------------
	_, err = payments.CreatePayment(ctx, &avav1.CreatePaymentRequest{BusinessId: tc.businessID, ContactId: customer.GetId(), PaymentType: "RECEIVED", PaymentNumber: "P-0", PaymentDate: dateOf(2026, 1, 10), Amount: dec("220.00"), PaymentMethod: "CASH",
		Applications: []*avav1.PaymentApplicationInput{{InvoiceId: inv.GetId(), AppliedAmount: dec("220.00")}}})
	wantCode(t, err, codes.FailedPrecondition) // still DRAFT
	inv = must(invoices.UpdateInvoiceStatus(ctx, &avav1.UpdateInvoiceStatusRequest{Id: inv.GetId(), Status: "SENT"})).GetInvoice()

	pay := must(payments.CreatePayment(ctx, &avav1.CreatePaymentRequest{
	...
	balanced(t, must(txns.GetLedgerTransaction(ctx, &avav1.GetLedgerTransactionRequest{Id: pay.GetLedgerTransactionId()})).GetTransaction())
	inv = must(invoices.GetInvoice(ctx, &avav1.GetInvoiceRequest{Id: inv.GetId()})).GetInvoice()
	...
	if listedPays := must(payments.ListPayments(ctx, &avav1.ListPaymentsRequest{BusinessId: tc.businessID})).GetPayments(); len(listedPays) != 1 || len(listedPays[0].GetApplications()) != 1 {
	...
	_, err = invoices.UpdateInvoiceLineItems(ctx, &avav1.UpdateInvoiceLineItemsRequest{Id: inv.GetId(), LineItems: []*avav1.NewDocumentLineItem{{ItemId: consult.GetId(), LineNumber: 1}}})
	wantCode(t, err, codes.FailedPrecondition)
	_, err = invoices.UpdateInvoiceStatus(ctx, &avav1.UpdateInvoiceStatusRequest{Id: inv.GetId(), Status: "CANCELLED"})
	wantCode(t, err, codes.FailedPrecondition)

	voided := must(payments.VoidPayment(ctx, &avav1.VoidPaymentRequest{Id: pay.GetId()})).GetPayment()
	if voided.GetId() != pay.GetId() {
		t.Fatal("void should echo the payment")
	}
	_, err = payments.GetPayment(ctx, &avav1.GetPaymentRequest{Id: pay.GetId()})
	...
	inv = must(invoices.UpdateInvoiceLineItems(ctx, &avav1.UpdateInvoiceLineItemsRequest{Id: inv.GetId(), ResourceVersion: inv.GetResourceVersion(),
		LineItems: []*avav1.NewDocumentLineItem{{ItemId: consult.GetId(), LineNumber: 1, Quantity: dec("3")}}})).GetInvoice()
	...
	reposted := must(txns.GetLedgerTransaction(ctx, &avav1.GetLedgerTransactionRequest{Id: posting.GetId()})).GetTransaction()
	if total := balanced(t, reposted); !total.Equal(decimal.RequireFromString("330")) {
		t.Fatalf("reposted total = %s, want 330", total)
	}

	cancelled := must(invoices.UpdateInvoiceStatus(ctx, &avav1.UpdateInvoiceStatusRequest{Id: inv.GetId(), Status: "CANCELLED", ReversalDate: dateOf(2026, 1, 20)})).GetInvoice()
	...
	_, err = invoices.UpdateInvoice(ctx, &avav1.UpdateInvoiceRequest{Id: inv.GetId(), Notes: ptr("x")})
	wantCode(t, err, codes.FailedPrecondition)
	if pdfBytes := must(invoices.GetInvoicePdf(ctx, &avav1.GetInvoicePdfRequest{Id: inv.GetId()})).GetContent(); !bytes.HasPrefix(pdfBytes, []byte("%PDF")) {
		t.Fatal("invoice pdf did not render")
	}

	// --- purchase invoice against the vendor --------------------------------
	purchase := must(invoices.CreateInvoice(ctx, &avav1.CreateInvoiceRequest{
		...
		LineItems: []*avav1.NewDocumentLineItem{{ItemId: supplies.GetId(), LineNumber: 1, Quantity: dec("4")}},
	})).GetInvoice()
	eqAmount(t, "purchase total", purchase.GetTotalAmount(), "200.00")
	balanced(t, must(txns.GetLedgerTransaction(ctx, &avav1.GetLedgerTransactionRequest{Id: purchase.GetLedgerTransactionId()})).GetTransaction())
	// Default list hides the cancelled one; --all shows both.
	if open := must(invoices.ListInvoices(ctx, &avav1.ListInvoicesRequest{BusinessId: tc.businessID})).GetInvoices(); len(open) != 1 {
		t.Fatalf("open invoices = %d, want 1", len(open))
	}
	if all := must(invoices.ListInvoices(ctx, &avav1.ListInvoicesRequest{BusinessId: tc.businessID, IncludeAll: true})).GetInvoices(); len(all) != 2 || len(all[0].GetLineItems()) != 1 {
		t.Fatalf("all invoices = %+v", all)
	}

	// --- raw posting and reversal -----------------------------------------
	opening := must(txns.CreateLedgerTransaction(ctx, &avav1.CreateLedgerTransactionRequest{
		BusinessId: tc.businessID, TransactionDate: dateOf(2026, 1, 2), Description: ptr("Opening balance"),
		Entries: []*avav1.NewLedgerEntry{{AccountId: cash, DebitAmount: dec("1000.00")}, {AccountId: equity, CreditAmount: dec("1000.00")}},
	})).GetTransaction()
	_, err = txns.CreateLedgerTransaction(ctx, &avav1.CreateLedgerTransactionRequest{BusinessId: tc.businessID, TransactionDate: dateOf(2026, 1, 2),
		Entries: []*avav1.NewLedgerEntry{{AccountId: cash, DebitAmount: dec("1")}, {AccountId: equity, CreditAmount: dec("2")}}})
	wantCode(t, err, codes.InvalidArgument) // unbalanced
	mistake := must(txns.CreateLedgerTransaction(ctx, &avav1.CreateLedgerTransactionRequest{
		BusinessId: tc.businessID, TransactionDate: dateOf(2026, 1, 3),
		Entries: []*avav1.NewLedgerEntry{{AccountId: expense, DebitAmount: dec("5.00")}, {AccountId: cash, CreditAmount: dec("5.00")}},
	})).GetTransaction()
	reversal := must(txns.ReverseLedgerTransaction(ctx, &avav1.ReverseLedgerTransactionRequest{Id: mistake.GetId()})).GetTransaction()
	if reversal.GetReversesLedgerTransactionId() != mistake.GetId() {
		t.Fatal("reversal should link back to the original")
	}
	_, err = txns.ReverseLedgerTransaction(ctx, &avav1.ReverseLedgerTransactionRequest{Id: mistake.GetId()})
	wantCode(t, err, codes.FailedPrecondition) // already reversed
	_, err = txns.ReverseLedgerTransaction(ctx, &avav1.ReverseLedgerTransactionRequest{Id: reversal.GetId()})
	wantCode(t, err, codes.FailedPrecondition) // a reversal can't be reversed
	if page := must(txns.ListLedgerTransactions(ctx, &avav1.ListLedgerTransactionsRequest{BusinessId: tc.businessID, PageSize: 2})).GetTransactions(); len(page) != 2 || len(page[0].GetEntries()) == 0 {
	...
	candidates := must(statements.ListUnreconciledLedgerTransactions(ctx, &avav1.ListUnreconciledLedgerTransactionsRequest{LedgerAccountId: cash, ThroughDate: dateOf(2026, 1, 31)})).GetTransactions()
	if len(candidates) < 3 { // opening, mistake, reversal, payment, void
		t.Fatalf("unreconciled = %d", len(candidates))
	}
	stmt = must(statements.ReconcileLedgerTransactions(ctx, &avav1.ReconcileLedgerTransactionsRequest{BankStatementId: stmt.GetId(), LedgerTransactionIds: []int64{opening.GetId()}})).GetBankStatement()
	eqAmount(t, "reconciled after", stmt.GetReconciledBalance(), "1000.00")
	eqAmount(t, "difference after", stmt.GetDifference(), "0")
	_, err = statements.ReconcileLedgerTransactions(ctx, &avav1.ReconcileLedgerTransactionsRequest{BankStatementId: stmt.GetId(), LedgerTransactionIds: []int64{est.GetId() + 1<<40}})
	wantCode(t, err, codes.InvalidArgument) // not this business's transaction
	_, err = statements.DeactivateBankStatement(ctx, &avav1.DeactivateBankStatementRequest{Id: stmt.GetId()})
	wantCode(t, err, codes.FailedPrecondition) // still has a line
	// A second statement must chain from the first's closing balance.
	_, err = statements.CreateBankStatement(ctx, &avav1.CreateBankStatementRequest{BusinessId: tc.businessID, LedgerAccountId: cash, StatementName: "Feb", StatementDate: dateOf(2026, 2, 28), OpeningBalance: dec("999"), ClosingBalance: dec("999")})
	wantCode(t, err, codes.InvalidArgument)
	if listed := must(statements.ListBankStatements(ctx, &avav1.ListBankStatementsRequest{BusinessId: tc.businessID})).GetBankStatements(); len(listed) != 1 || len(listed[0].GetLines()) != 1 {
		t.Fatalf("list statements should carry lines and balances: %+v", listed)
	}
	stmt = must(statements.UnreconcileLedgerTransactions(ctx, &avav1.UnreconcileLedgerTransactionsRequest{BankStatementId: stmt.GetId(), LedgerTransactionIds: []int64{opening.GetId()}})).GetBankStatement()
	must(statements.DeactivateBankStatement(ctx, &avav1.DeactivateBankStatementRequest{Id: stmt.GetId()}))

	// --- reports -----------------------------------------------------------
	tb := must(reports.GetTrialBalance(ctx, &avav1.GetTrialBalanceRequest{BusinessId: tc.businessID, AsOf: dateOf(2026, 12, 31)})).GetTrialBalance()
	...
	is := must(reports.GetIncomeStatement(ctx, &avav1.GetIncomeStatementRequest{BusinessId: tc.businessID, PeriodStart: dateOf(2026, 1, 1), PeriodEnd: dateOf(2026, 12, 31)})).GetIncomeStatement()
	eqAmount(t, "total expenses", is.GetTotalExpenses(), "200.00") // the purchase; the 5.00 mistake was reversed
	gl := must(reports.GetGeneralLedger(ctx, &avav1.GetGeneralLedgerRequest{BusinessId: tc.businessID, AccountId: cash, PeriodStart: dateOf(2026, 1, 1), PeriodEnd: dateOf(2026, 12, 31)})).GetGeneralLedger()
	eqAmount(t, "cash ending balance", gl.GetEndingBalance(), "1000.00")
	must(reports.GetBalanceSheet(ctx, &avav1.GetBalanceSheetRequest{BusinessId: tc.businessID, AsOf: dateOf(2026, 12, 31)}))
	stmtReport := must(reports.GetCustomerStatement(ctx, &avav1.GetCustomerStatementRequest{ContactId: customer.GetId(), PeriodStart: dateOf(2026, 1, 1), PeriodEnd: dateOf(2026, 12, 31)})).GetStatement()
	if stmtReport.GetContactName() != "Acme" {
		t.Fatalf("customer statement for %q", stmtReport.GetContactName())
	}
	_, err = reports.GetCustomerStatement(tc.outsider, &avav1.GetCustomerStatementRequest{ContactId: customer.GetId(), PeriodStart: dateOf(2026, 1, 1), PeriodEnd: dateOf(2026, 12, 31)})
	wantCode(t, err, codes.NotFound)
	_, err = reports.GetTrialBalance(ctx, &avav1.GetTrialBalanceRequest{BusinessId: tc.businessID})
	wantCode(t, err, codes.InvalidArgument) // as_of required
	for name, content := range map[string][]byte{
		"trial balance":      must(reports.GetTrialBalancePdf(ctx, &avav1.GetTrialBalancePdfRequest{BusinessId: tc.businessID, AsOf: dateOf(2026, 12, 31)})).GetContent(),
		"balance sheet":      must(reports.GetBalanceSheetPdf(ctx, &avav1.GetBalanceSheetPdfRequest{BusinessId: tc.businessID, AsOf: dateOf(2026, 12, 31)})).GetContent(),
		"income statement":   must(reports.GetIncomeStatementPdf(ctx, &avav1.GetIncomeStatementPdfRequest{BusinessId: tc.businessID, PeriodStart: dateOf(2026, 1, 1), PeriodEnd: dateOf(2026, 12, 31)})).GetContent(),
		"general ledger":     must(reports.GetGeneralLedgerPdf(ctx, &avav1.GetGeneralLedgerPdfRequest{BusinessId: tc.businessID, AccountId: cash, PeriodStart: dateOf(2026, 1, 1), PeriodEnd: dateOf(2026, 12, 31)})).GetContent(),
		"customer statement": must(reports.GetCustomerStatementPdf(ctx, &avav1.GetCustomerStatementPdfRequest{ContactId: customer.GetId(), PeriodStart: dateOf(2026, 1, 1), PeriodEnd: dateOf(2026, 12, 31)})).GetContent(),
	} {
	...
	// --- period close --------------------------------------------------------
	pc := must(closes.TriggerClose(ctx, &avav1.TriggerCloseRequest{BusinessId: tc.businessID, PeriodEnd: dateOf(2026, 1, 31)})).GetPeriodClose()
	if len(pc.GetGeneratedLedgerTransactionIds()) == 0 {
		t.Fatal("close should have generated closing entries")
	}
	_, err = txns.CreateLedgerTransaction(ctx, &avav1.CreateLedgerTransactionRequest{BusinessId: tc.businessID, TransactionDate: dateOf(2026, 1, 15),
	...
	if listed := must(closes.ListPeriodCloses(ctx, &avav1.ListPeriodClosesRequest{BusinessId: tc.businessID})).GetPeriodCloses(); len(listed) != 1 || len(listed[0].GetGeneratedLedgerTransactionIds()) != len(pc.GetGeneratedLedgerTransactionIds()) {
		t.Fatalf("list closes should carry generated transactions: %+v", listed)
	}
	reversedClose := must(closes.ReverseClose(ctx, &avav1.ReverseCloseRequest{Id: pc.GetId()})).GetPeriodClose()
	...
	wantCode(t, err, codes.InvalidArgument)
	_, err = notes.CreateEntityContext(ctx, &avav1.CreateEntityContextRequest{BusinessId: tc.businessID, EntityType: "spaceship", EntityId: 1, ContextType: "user_note", Content: "x"})
	wantCode(t, err, codes.InvalidArgument)
	newer := must(notes.CreateEntityContext(ctx, &avav1.CreateEntityContextRequest{BusinessId: tc.businessID, EntityType: "invoice", EntityId: inv.GetId(), ContextType: "user_note", Content: "discount declined", SupersedesIds: []int64{note.GetId()}})).GetEntityContext()
	if current := must(notes.ListEntityContext(ctx, &avav1.ListEntityContextRequest{BusinessId: tc.businessID, EntityType: "invoice", EntityId: inv.GetId()})).GetEntityContexts(); len(current) != 1 || current[0].GetId() != newer.GetId() {
		t.Fatalf("superseded note should be hidden: %+v", current)
	}
	must(notes.DeleteEntityContext(ctx, &avav1.DeleteEntityContextRequest{Id: newer.GetId()}))
	if current := must(notes.ListEntityContext(ctx, &avav1.ListEntityContextRequest{BusinessId: tc.businessID, EntityType: "invoice", EntityId: inv.GetId()})).GetEntityContexts(); len(current) != 1 || current[0].GetId() != note.GetId() {
	...
	all := must(contacts.ListContacts(ctx, &avav1.ListContactsRequest{BusinessId: tc.businessID})).GetContacts()
	roles := 0
	for _, c := range all {
