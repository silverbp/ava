...
func TestSystemAccounts_PersistOnBusiness(t *testing.T) {
	...
	custAcct := must(newLedgerAccountService(tc.store).GetLedgerAccount(ctx, &avav1.GetLedgerAccountRequest{Id: customer.GetCustomer().GetLedgerAccountId()})).GetAccount()
	...
	vendAcct := must(newLedgerAccountService(tc.store).GetLedgerAccount(ctx, &avav1.GetLedgerAccountRequest{Id: vendor.GetVendor().GetLedgerAccountId()})).GetAccount()
	...
	otherAcct := must(newLedgerAccountService(tc.store).GetLedgerAccount(ctx, &avav1.GetLedgerAccountRequest{Id: other.GetCustomer().GetLedgerAccountId()})).GetAccount()
	...
	must(closes.TriggerClose(ctx, &avav1.TriggerCloseRequest{BusinessId: tc.businessID, PeriodEnd: dateOf(2026, 1, 31)}))
	biz, err = tc.q.GetBusiness(ctx, tc.businessID)
	if err != nil {
		t.Fatalf("GetBusiness: %v", err)
	}
...
func TestContact_DuplicateNamesAllowed(t *testing.T) {
	...
	first := must(contacts.CreateContact(ctx, &avav1.CreateContactRequest{
		BusinessId: tc.businessID, ContactNumber: "C-1", Name: "John Smith", IsCustomer: true,
	})).GetContact()
	second := must(contacts.CreateContact(ctx, &avav1.CreateContactRequest{
	...
	must(contacts.DeactivateContact(ctx, &avav1.DeactivateContactRequest{Id: first.GetId()}))
	must(contacts.CreateContact(ctx, &avav1.CreateContactRequest{
...
func TestContact_SelfHealRejectsMismatchedContainer(t *testing.T) {
	...
	_, err := contacts.CreateContact(ctx, &avav1.CreateContactRequest{
		BusinessId: tc.businessID, ContactNumber: "C-1", Name: "Acme Co", IsCustomer: true,
	})
