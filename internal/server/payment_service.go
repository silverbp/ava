...
func (s *paymentService) ListPayments(ctx context.Context, req *avav1.ListPaymentsRequest) (*avav1.ListPaymentsResponse, error) {
	...
	return &avav1.ListPaymentsResponse{Payments: pbs}, nil
}
...
func (s *paymentService) CreatePayment(ctx context.Context, req *avav1.CreatePaymentRequest) (*avav1.CreatePaymentResponse, error) {
	...
	return &avav1.CreatePaymentResponse{Payment: paymentWithApplications(payment, applications)}, nil
}
...
func paymentToProto(ctx context.Context, q *sqlcgen.Queries, p sqlcgen.Payment) (*avav1.Payment, error) {
	return one(paymentsToProto(ctx, q, []sqlcgen.Payment{p}))
}

// paymentsToProto converts a page of payments with their applications
// loaded in one query.
func paymentsToProto(ctx context.Context, q *sqlcgen.Queries, rows []sqlcgen.Payment) ([]*avav1.Payment, error) {
...
// already holds.
func paymentWithApplications(p sqlcgen.Payment, applications []sqlcgen.PaymentApplication) *avav1.Payment {
	pbApplications := make([]*avav1.PaymentApplication, len(applications))
	for i, a := range applications {
		pbApplications[i] = &avav1.PaymentApplication{
			Id:            a.ID,
