...
func TestContact_CustomerAndVendorRoles(t *testing.T) {
	...
	both := must(contacts.CreateContact(ctx, &avav1.CreateContactRequest{
	...
	neither := must(contacts.CreateContact(ctx, &avav1.CreateContactRequest{
	...
	updated := must(contacts.UpdateContact(ctx, &avav1.UpdateContactRequest{
	...
	deactivated := must(contacts.DeactivateContact(ctx, &avav1.DeactivateContactRequest{
	...
	got := must(contacts.GetContact(ctx, &avav1.GetContactRequest{Id: both.GetId()})).GetContact()
	if got.GetIsActive() {
	...
	_, err := contacts.DeactivateContact(ctx, &avav1.DeactivateContactRequest{
		Id: both.GetId(), ResourceVersion: updated.GetResourceVersion(),
	})
...
func TestContact_AutoProvisionsLedgerAccount(t *testing.T) {
	...
	customer := must(contacts.CreateContact(ctx, &avav1.CreateContactRequest{
	...
	arAcct := must(accounts.GetLedgerAccount(ctx, &avav1.GetLedgerAccountRequest{Id: custAR})).GetAccount()
	...
	arContainer := must(accounts.GetLedgerAccount(ctx, &avav1.GetLedgerAccountRequest{Id: arAcct.GetParentAccountId()})).GetAccount()
	...
	vendor := must(contacts.CreateContact(ctx, &avav1.CreateContactRequest{
		BusinessId: tc.businessID, ContactNumber: "V-1", Name: "Supplies Co", IsVendor: true,
	})).GetContact()
	vendAP := vendor.GetVendor().GetLedgerAccountId()
	apAcct := must(accounts.GetLedgerAccount(ctx, &avav1.GetLedgerAccountRequest{Id: vendAP})).GetAccount()
	if apAcct.GetName() != "Supplies Co" || apAcct.GetAccountTypeId() != 2 {
		t.Fatalf("unexpected auto-provisioned vendor account: %+v", apAcct)
	}
	apContainer := must(accounts.GetLedgerAccount(ctx, &avav1.GetLedgerAccountRequest{Id: apAcct.GetParentAccountId()})).GetAccount()
	...
	other := must(contacts.CreateContact(ctx, &avav1.CreateContactRequest{
		BusinessId: tc.businessID, ContactNumber: "C-2", Name: "Beta Inc", IsCustomer: true,
	})).GetContact()
	otherAcct := must(accounts.GetLedgerAccount(ctx, &avav1.GetLedgerAccountRequest{Id: other.GetCustomer().GetLedgerAccountId()})).GetAccount()
...
func TestDeactivateContact_VendorLedgerAccount(t *testing.T) {
	...
	vendAP := vendor.GetVendor().GetLedgerAccountId()
	item := must(items.CreateItem(ctx, &avav1.CreateItemRequest{
	...
	inv := must(invoices.CreateInvoice(ctx, &avav1.CreateInvoiceRequest{
		...
		LineItems: []*avav1.NewDocumentLineItem{{ItemId: item.GetId(), LineNumber: 1}},
	})).GetInvoice()

	must(contacts.DeactivateContact(ctx, &avav1.DeactivateContactRequest{Id: vendor.GetId()}))
	acct := must(accounts.GetLedgerAccount(ctx, &avav1.GetLedgerAccountRequest{Id: vendAP})).GetAccount()
	if !acct.GetIsActive() {
